...
这一版不需要常驻服务器。阿里云函数计算 FC 负责准时唤醒，GitHub Actions 每天在云端启动临时运行器完成采集、验证、筛选和静态页面生成；GitHub Pages 保存并提供最后一次成功生成的页面。电脑和手机都可以关机。

## 需要准备的账号与设置

需要 GitHub、DeepSeek API 和已开通函数计算的阿里云账号；邮件推送还需要已开启 SMTP 的 163 邮箱。新闻 RSS、Hacker News、掘金与 arXiv 采集本身不需要 API Key。知乎、CSDN 和微信公众号本版未启用，因此当前部署不需要新增任何社区账号或 Secret。
...
不要把 DeepSeek Key、GitHub 密码、Personal Access Token、邮箱密码或 SMTP 授权码写入文件、Issue 或聊天。GitHub Pages 发布使用 Actions 自动提供的短期 `GITHUB_TOKEN`；阿里云外部触发需要的仓库级 fine-grained token 只保存在 FC 环境变量或 KMS 中，设置方法见 [`ALIYUN_FC_DISPATCHER.md`](ALIYUN_FC_DISPATCHER.md)。
...
```text
阿里云 FC 每 10 分钟检查一次；07:00 起自动进入新闻预处理阶段
        ↓
抓取并验证新闻 → DeepSeek 新闻筛选 → 保存当天状态，不发邮件
        ↓
阿里云 FC 达到 arXiv 公告边界后自动进入发布阶段（自动适配美东夏令时）
        ↓
...
阿里云 FC 每 10 分钟的唤醒是状态检查，不是重复采集：北京时间 07:00–10:30 之外会立即结束且不访问网络；窗口内先检查当天 Pages、同阶段成功记录和运行中的工作流，只有确实需要时才调用 GitHub `workflow_dispatch`。GitHub 自带的 08:10、08:30、08:50、09:10、09:30 五个 `schedule` 继续作为外部服务失效时的独立兜底。成功标记按北京时间日期保存在 GitHub Actions Cache；第一次完整成功后，后续运行会在安装依赖和调用 DeepSeek 前退出。工作流的 `cancel-in-progress` 为 `false`，所以较晚到达的备用事件只会排队，不会取消正在运行的主任务。

arXiv 的正式公告时间是美东时间 20:00，对应北京时间夏令时 08:00、冬令时 09:00。FC 调度函数按 `America/New_York` 时区动态判断，公告时间之前不会触发论文阶段；采集器还会再次执行就绪检查。任何过早尝试都不会把上一批论文冒充今日结果，也不会花费论文筛选 Token。

拆成两阶段后，通常约 20 分钟的新闻流程在 08 点前完成，公告后只剩约 10–15 分钟的论文与发布流程。若预处理失败，`publish` 阶段会自动补采新闻；若邮件或 Pages 在采集后失败，后续重试会复用当天成功的新闻与论文状态。

手动运行默认也遵守当日去重。若确实需要在当天重新采集，可在 **Run workflow** 时勾选 `force`；这可能产生新的 DeepSeek 调用，并会再次发送邮件，应只在明确需要时使用。

跨天补跑请同时设置 `phase=publish`、`force=true`、`target_date=YYYY-MM-DD`，日期必须是北京时间的过去某一天，例如 `2026-09-25`。留空 `target_date` 则保持原有当日运行方式。跨天补跑有以下约束：

- 必须恢复目标日已完成、且 Prompt 版本一致的新闻状态；沿用原新闻采集时间计算回看窗口，不用今天的 RSS 替代历史新闻。缺失新闻快照会在付费论文调用之前失败。
- arXiv 查询使用目标日的公告窗口；如果 API 不可用，官方 `/new` 备用列表也必须仍对应目标日。列表已滚动到其他日期时会停止，不伪造历史结果。
- 补跑发布到 `archive/YYYY-MM-DD/`，首页增加历史入口，原首页快照及其更新时间保持不变，避免今日定时任务误以为已发布。后续正常发布会保留这些历史归档；归档读取失败时停止部署，避免误删。
- 邮件标注目标日期和“补发”，成功标记归属于目标日；新调用费用仍计入实际执行日，会占用执行日的正常预算。缓存优先恢复执行日账本，其次恢复目标日状态，不清零失败调用的费用。

若已确认允许本次补跑使用更高额度，可同时勾选 `rerun_budget`：仅在手动且 `force=true` 的该次运行中，将当天累计上限设为 700,000 Token / $1.50。此前用量仍计入，配置文件中的日常 500,000 Token / $1 上限不变；后续定时任务不会自动获得更高额度。

页面上的开关是只读的前端筛选：

- `AI 重大发布与热议 / 自动驾驶-VLN 论文`（VLN 包括自动驾驶和室内导航）
- `精选 / 全部`
- 论文 `今日 / 近 4 日`
- 标题、摘要、作者和标签搜索
...
已通过校验的决定也会逐条存入同一状态缓存，覆盖新闻、论文初筛和论文复筛。仅复用候选输入、模型、推理配置、Prompt 版本和内容均一致的决定；严格原文摘录校验失败时，会记录对应论文和不支持的引文并单独重试。无效决定不写入断点，未完成整轮筛选也不发布部分日报。
...
- GitHub 自带的备用定时仍可能因高负载延迟；10 点前交付依赖已正确部署、能够访问 GitHub 且 Token 未过期的阿里云 FC 函数，也不构成绝对服务等级保证。
- GitHub 可能在公开仓库长期无活动后停用原生定时；阿里云 FC 的 `workflow_dispatch` 应定期检查，Token 过期前需要轮换。
...
- [阿里云函数计算定时触发器](https://help.aliyun.com/zh/functioncompute/time-triggers)
